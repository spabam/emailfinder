#!/bin/bash
# TheHarvester Email Checker Setup Script
# =======================================
# Copyright (c) 2015-2035 Andrea Bodei - info@andreabodei.com
# Version: 2.0.1.202510210253
# 
# This script sets up the TheHarvester email checker environment.
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║                    TheHarvester Email Checker Setup                         ║${NC}"
echo -e "${BLUE}║                                                                              ║${NC}"
echo -e "${BLUE}║  Copyright (c) 2015-2035 Andrea Bodei - info@andreabodei.com                ║${NC}"
echo -e "${BLUE}║  Version: 2.0.1.202510210253                                                  ║${NC}"
echo -e "${BLUE}║                                                                              ║${NC}"
echo -e "${BLUE}║  This script sets up the TheHarvester email checker environment.            ║${NC}"
echo -e "${BLUE}║                                                                              ║${NC}"
...
PYTHON_SCRIPT="$SCRIPT_DIR/theharvester_checker.py"

if [ -f "$PYTHON_SCRIPT" ]; then
    echo -e "${GREEN}✅ TheHarvester checker script found${NC}"
    
    # Test script execution
    if python3 "$PYTHON_SCRIPT" --banner >/dev/null 2>&1; then
        echo -e "${GREEN}✅ TheHarvester checker script is working${NC}"
    else
        echo -e "${RED}❌ TheHarvester checker script has issues${NC}"
        exit 1
    fi
else
    echo -e "${RED}❌ TheHarvester checker script not found${NC}"
    exit 1
fi

# Test runner script
RUNNER_SCRIPT="$SCRIPT_DIR/run_theharvester_checker.sh"

if [ -f "$RUNNER_SCRIPT" ]; then
    echo -e "${GREEN}✅ TheHarvester checker runner script found${NC}"
    
    # Make runner script executable
    chmod +x "$RUNNER_SCRIPT"
    echo -e "${GREEN}✅ TheHarvester checker runner script is executable${NC}"
else
    echo -e "${RED}❌ TheHarvester checker runner script not found${NC}"
    exit 1
fi

# Test TheHarvester installation
echo -e "${BLUE}🔍 Testing TheHarvester installation...${NC}"
if command -v theharvester &> /dev/null; then
    echo -e "${GREEN}✅ TheHarvester is accessible${NC}"
else
    echo -e "${YELLOW}⚠️  TheHarvester not found in PATH${NC}"
...
echo ""
echo -e "${GREEN}🎉 TheHarvester Email Checker setup completed successfully!${NC}"
echo ""
echo -e "${BLUE}📋 USAGE EXAMPLES:${NC}"
echo -e "${YELLOW}  # Basic TheHarvester email search${NC}"
echo -e "  ./run_theharvester_checker.sh example.com"
echo ""
echo -e "${YELLOW}  # With output file${NC}"
echo -e "  ./run_theharvester_checker.sh example.com --output emails.txt"
echo ""
echo -e "${YELLOW}  # JSON output${NC}"
echo -e "  ./run_theharvester_checker.sh example.com --output emails.json"
echo ""
echo -e "${YELLOW}  # Verbose output${NC}"
echo -e "  ./run_theharvester_checker.sh example.com --verbose"
echo ""
echo -e "${YELLOW}  # Show banner${NC}"
echo -e "  ./run_theharvester_checker.sh --banner"
echo ""
echo -e "${BLUE}📚 For more information, run:${NC}"
echo -e "  ./run_theharvester_checker.sh --help"
echo ""
echo -e "${GREEN}✅ Setup complete! You can now use the TheHarvester Email Checker.${NC}"
