#!/bin/bash
# GPG/PGP Email Checker Runner Script
# ===================================
# Copyright (c) 2015-2035 Andrea Bodei - info@andreabodei.com
# Version: 2.0.1.202510210253
# 
# This script runs the GPG/PGP email checker with proper
...
if [ ! -f "$PYTHON_SCRIPT" ]; then
    echo -e "${RED}❌ Error: GPG email checker script not found at: $PYTHON_SCRIPT${NC}"
    exit 1
fi

# Function to show usage
show_usage() {
    echo "GPG/PGP Email Checker Runner Script"
    echo "===================================="
    echo ""
    echo "Copyright (c) 2015-2035 Andrea Bodei - info@andreabodei.com"
    echo ""
    echo "This script runs the GPG/PGP email checker with proper"
    ...
    echo "  - Internet connection for GPG key server access"
    echo ""
    echo "Copyright (c) 2015-2035 Andrea Bodei - info@andreabodei.com"
}
...
if [ "$1" = "--banner" ]; then
    if [ ! -x "$VENV_PY" ]; then
        ...
        echo -e "${BLUE}💡 Please run ./setup_gpg.sh first${NC}"
        exit 1
    fi
...
# Run the GPG email checker
echo -e "${BLUE}🚀 Starting GPG/PGP Email Checker...${NC}"
echo ""
...
if [ $? -eq 0 ]; then
    echo ""
    echo -e "${GREEN}✅ GPG email search completed successfully${NC}"
else
    echo ""
    echo -e "${RED}❌ GPG email search failed${NC}"
    exit 1
fi
